	...
	winStart(wS), winLen(wL)
{
	...
	for (size_t f = 0; f < files.size(); f++){
		...
		while(getline(shapeFile,line)){		// Each line in the shape files represent a single site
			...
			// Populate the row f of the matrix corresponding to this shape file
	...
	// Scale matrix values to go from 0->1
	for (size_t i = 0; i < nSites; i++)
		scaleMatrix(matrices[i]);
...
alignData shapeAlign::getOptimalShift(gsl_matrix *A1, gsl_matrix *A2){
	...
	results.score = DBL_MAX;	// Initialize results.score to a large number
	double score;
	...
	for (int s = shiftMin; s <= shiftMax; s++){
		...
		gsl_matrix *delta = gsl_matrix_alloc(m,A2->size2-abs(s));
		
		// Get the values that fall within the overlapping region as
		// submatrices
		gsl_matrix_view subA1 = (s <= 0 ? gsl_matrix_submatrix(A1,0,0,m,A2->size2+s)
			: gsl_matrix_submatrix(A1,0,s,m,A2->size2-s) );
		gsl_matrix_view subA2 = ( s <= 0 ? gsl_matrix_submatrix(A2,0,abs(s),m,A2->size2+s)
		...
		//
		//
		//			NEED TO FIX THE WAY THESE WINDOWS ARE DEFINED
		//
		//
		//
		//
		...
		// These parameters represent the window in which the alignment
		// is performed. 0 to M->size2, for example would represent all
		// possible values contributing to the distance computation
		...
		if ( winLen == -1){
			...
			s <= 0 ? iMin = winStart : iMin = winStart - s;
			iMax = iMin + winLen;
		}
		
		// Compute the normalized Frobenius distance
		score = normFrobenius(delta,iMin,iMax);
...
double shapeAlign::normFrobenius(const gsl_matrix *M, const int &iMin,
	const int &iMax)
{
	double L2n, Fn = 0;
	...
	return sqrt(Fn/(iMax-iMin));
}


// Scale each row in a matrix to have values in the 0-1 range
void shapeAlign::scaleMatrix(gsl_matrix *M){
	for (size_t i = 0; i < M->size1; i++){
		double min,max;
		gsl_vector_view row = gsl_matrix_row(M,i);
		gsl_vector_minmax(&row.vector,&min,&max);
		gsl_vector_add_constant(&row.vector,-1*min);
		gsl_vector_scale(&row.vector,1/(max-min));
	}
	return;
}
